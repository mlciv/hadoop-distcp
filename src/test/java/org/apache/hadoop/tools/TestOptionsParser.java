...
public class TestOptionsParser {
  ...
  public void testToString() {
    ...
    String val = "DistCpOptions{atomicCommit=false, syncFolder=false, deleteMissing=false, " +
        ...
        "preserveRawXattrs=false, byChunk=false, maxThreads=-1}";
  ...
  @Test
  public void testByChunkOption(){
    Configuration conf = new Configuration();
    Assert.assertFalse(conf.getBoolean(DistCpOptionSwitch.BY_CHUNK.getConfigLabel(),false));

    DistCpOptions options = OptionsParser.parse(new String[] {
            "hdfs://localhost:8020/source/first",
            "hdfs://localhost:8020/target/"});
    Assert.assertFalse(options.getByChunk());

    options = OptionsParser.parse(new String[] {
            "-bychunk",
            "hdfs://localhost:8020/source/first",
            "hdfs://localhost:8020/target/"});
    Assert.assertTrue(options.getByChunk());
  }
}
