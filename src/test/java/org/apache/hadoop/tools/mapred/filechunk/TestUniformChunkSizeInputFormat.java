...
public class TestUniformChunkSizeInputFormat {
  ...
  public void testGetChunkSplits(int nMaps) throws Exception {
    ...
    UniformChunkSizeInputFormat uniformSizeInputFormat = new UniformChunkSizeInputFormat();
    List<InputSplit> splits
            = uniformSizeInputFormat.getSplits(jobContext);

    int sizePerMap = totalFileSize/nMaps;

    checkChunkSplits(listFile, splits);
    ...
    for (int i=0; i<splits.size(); ++i) {
      ...
      while (recordReader.nextKeyValue()) {
        CopyListingChunkFileStatus valueFileStatus = recordReader.getCurrentValue();
        Path sourcePath = valueFileStatus.getPath();
        //TODO any problem for not recheck filestatus?
        if(valueFileStatus.isDirectory()) {
          continue;
        }
        else{
          currentSplitSize += valueFileStatus.getCopyLength();
        }
      }
      // verify per map  per chunk
      Assert.assertTrue(
              previousSplitSize == -1
  ...
  private void checkChunkSplits(Path listFile, List<InputSplit> splits) throws IOException {
    long lastEnd = 0;
  ...
  @Test
  public void testGetChunkSplits() throws Exception {
    testGetChunkSplits(9);
    for (int i=1; i<N_FILES; ++i)
      testGetChunkSplits(i);
  }
}
