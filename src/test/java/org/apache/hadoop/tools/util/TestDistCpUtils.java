...
public class TestDistCpUtils {
  ...
  @Test
  public void testGetChunkRelativePath(){
    Path root = new Path("/tmp/abc");
    Path child = new Path("/tmp/abc/xyz/file");
    Assert.assertEquals("/xyz/file_0000000122_chunkfiles/0000000012",DistCpUtils.getChunkRelativePath(root,child,12,122));
  }

  @Test
  public void testChangeChunkToOriginalPath() throws IOException{
    String relPath = "/xyz/file_0000000122_chunkfiles/0000000012";
    Assert.assertEquals("/xyz/file",DistCpUtils.changeChunkToOriginalPath(relPath));
  }

  @Test
  public void testGetBlockSize() throws IOException{
    EnumSet<FileAttribute> fileAttributes = DistCpUtils.unpackAttributes("bc");
    Path sourcePath = new Path("/tmp/source");
    Path targetPath = new Path("/tmp/target");
    FileSystem fs = cluster.getFileSystem();
    TestDistCpUtils.createFile(fs, sourcePath, (long) 1024, (short)2,(long)32*1024);
    Assert.assertEquals(32 * 1024, DistCpUtils.getBlockSize(fileAttributes, fs.getFileStatus(sourcePath), fs, targetPath));

    fileAttributes.remove(FileAttribute.BLOCKSIZE);
    Assert.assertEquals(32 * 1024,DistCpUtils.getBlockSize(fileAttributes, fs.getFileStatus(sourcePath), fs, targetPath));
    fileAttributes.remove(FileAttribute.CHECKSUMTYPE);
    Assert.assertEquals(fs.getDefaultBlockSize(targetPath),DistCpUtils.getBlockSize(fileAttributes, fs.getFileStatus(sourcePath), fs, targetPath));
  }
   ...
   * @throws IOException
   */
  public static void createFile(FileSystem fs, Path filePath,long size,short replic,long blockSize) throws IOException {
    OutputStream os = fs.create(filePath, true,1024,replic,blockSize);
    os.write(ByteBuffer.allocate((int) size).array());
    os.flush();
  ...
  @Test
  public void testGetBlocksPerChunk(){
    Configuration conf = new Configuration();
    Assert.assertEquals(DistCpConstants.DEFAULT_BLOCKS_PER_CHUNK,DistCpUtils.getBlocksPerChunk(conf));
    conf.set(DistCpConstants.CONF_LABEL_BLOCKS_PER_CHUNK, "2");
    Assert.assertEquals(2,DistCpUtils.getBlocksPerChunk(conf));
    conf.set(DistCpConstants.CONF_LABEL_BLOCKS_PER_CHUNK,"-1");
    Assert.assertEquals(DistCpConstants.DEFAULT_BLOCKS_PER_CHUNK,DistCpUtils.getBlocksPerChunk(conf));
  }
}
