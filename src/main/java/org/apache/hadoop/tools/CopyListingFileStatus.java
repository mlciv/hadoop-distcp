 ...
 */
//TODO : compatiable with older version hadoop without ACL
...
public class CopyListingFileStatus extends FileStatus {
  ...
  public CopyListingFileStatus(FileStatus fileStatus) throws IOException {
    // super(fileStatus);
    // for 2.4, not this constructor
    super(fileStatus.getLen(), fileStatus.isDirectory(), fileStatus.getReplication(),
  ...
  /**
   * Creates a new CopyListingFileStatus by copying the members of the given CopyListFileStatus
   * @param that
   * @throws IOException
   */
  public CopyListingFileStatus(CopyListingFileStatus that) throws IOException{
    // super(fileStatus);
    // for 2.4, not this constructor
    super(that.getLen(), that.isDirectory(), that.getReplication(),
            that.getBlockSize(), that.getModificationTime(), that.getAccessTime(),
            that.getPermission(), that.getOwner(), that.getGroup(),
            (that.isSymlink() ? that.getSymlink() : null),
            that.getPath());
    this.aclEntries = that.aclEntries;
    this.xAttrs = that.xAttrs;
  }
  ...
  /**
   * return length of this copy, can be override by chunk or other format
   */
