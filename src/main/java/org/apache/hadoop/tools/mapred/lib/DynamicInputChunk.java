...
class DynamicInputChunk<K, V> {
  ...
  //TODO: checkInitial by chunkRootPath
  private static boolean areInvariantsInitialized() {
    return chunkRootPath != null;
  }

  //TODO: For testing static variable
  public static void resetForTesting(){
    chunkRootPath = null;
    configuration = null;
    chunkFilePrefix = null;
    numChunksLeft = -1;
    fs = null;
  }
  ...
  //TODO: should be overrided
  private void openForWriteByChunk() throws IOException {
    writer = SequenceFile.createWriter(
            chunkFilePath.getFileSystem(configuration), configuration,
            chunkFilePath, Text.class, CopyListingChunkFileStatus.class,
            SequenceFile.CompressionType.NONE);

  }

  //TODO: for parallel
  private void openForWriteByParallel() throws IOException{
    writer = SequenceFile.createWriter(
            chunkFilePath.getFileSystem(configuration), configuration,
            chunkFilePath, Text.class, CopyListingChunkFileStatus.class,
            SequenceFile.CompressionType.NONE);
   ...
   * @throws IOException Exception onf failure to write to the file.
   */
  //TODO: write CopyListingChunkFileStatus,should be overrided
  public void write(Text key, CopyListingChunkFileStatus value) throws IOException {
    writer.append(key, value);
  }
  /**
