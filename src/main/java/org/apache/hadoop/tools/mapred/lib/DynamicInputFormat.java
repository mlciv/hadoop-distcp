...
public class DynamicInputFormat<K, V> extends InputFormat<K, V> {
  ...
  public List<InputSplit> getSplits(JobContext jobContext)
    ...
    LOG.info("DynamicInputFormat: Getting splits for job:"
             + jobContext.getJobID());
    List<DynamicInputChunk> lists = new ArrayList<DynamicInputChunk>();
    if(jobContext.getConfiguration().getBoolean(DistCpConstants.CONF_LABEL_COPY_BY_CHUNK,false)){
      lists = splitCopyListingIntoChunksWithShuffleByChunk(jobContext);
      //TODO: add parall
    }else {
      lists = splitCopyListingIntoChunksWithShuffle(jobContext);
    }
  ...
  private List<DynamicInputChunk> splitCopyListingIntoChunksWithShuffleByChunk
          (JobContext context) throws IOException {
    ...
    try {
      //dynamic chunks can not promise equal size
      while (reader.next(relPath, fileStatus)) {
        if (recordCounter % (nChunksOpenAtOnce*numEntriesPerChunk) == 0) {
          // All chunks full. Create new chunk-set.
          closeAll(openChunks);
  ...
  private List<DynamicInputChunk> splitCopyListingIntoChunksWithShuffleByParallel
          (JobContext context) throws IOException {
