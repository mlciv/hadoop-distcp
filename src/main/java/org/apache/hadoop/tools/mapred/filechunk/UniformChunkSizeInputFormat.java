 ...
 * uniform.
 */
public class UniformChunkSizeInputFormat
    extends InputFormat<Text, CopyListingChunkFileStatus> {
  private static final Log LOG
                = LogFactory.getLog(UniformChunkSizeInputFormat.class);
  ...
  private List<InputSplit> getSplits(Configuration configuration, int numSplits,
    ...
    LOG.info("Number of UniformChunkSize file split created: " + splits.size());
    return splits;
  }
   ...
   */
  @Override
  public RecordReader<Text, CopyListingChunkFileStatus> createRecordReader(
      InputSplit split, TaskAttemptContext context)
      throws IOException, InterruptedException {
    return new SequenceFileRecordReader<Text, CopyListingChunkFileStatus>();
  }
}
