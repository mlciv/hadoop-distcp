...
public class RetriableFileCopyCommand extends RetriableCommand {
  ...
  //TODO: multi Files in one map simultaneously,adding Name as suffix
  private Path getTmpFile(Path target, Mapper.Context context) {
    ...
    LOG.info("Creating temp file: " +
        new Path(root, ".distcp.tmp." + context.getTaskAttemptID().toString()+"_"+target.getName()));
    return new Path(root, ".distcp.tmp." + context.getTaskAttemptID().toString()+"_"+target.getName());
  }
  ...
  long copyBytes(FileStatus sourceFileStatus, long sourceOffset,
    ...
    try {
      ...
      //HDFS and HFTP should have the same read meaning.
...
//        LOG.info(String.format("coping file(%s),totalCopyLength=%d,totalBytesRead=%d,sourceOffset=%d,bytesRead=%d,bufLength=%d",sourceFileStatus.toString(),sourceFileStatus.getLen(),totalBytesRead,sourceOffset,bytesRead,buf.length));
      }
      LOG.info(String.format("copied file(%s),totalCopyLength=%d,totalBytesRead=%d,sourceOffset=%d,bytesRead=%d,bufLength=%d", sourceFileStatus.toString(), sourceFileStatus.getLen(), totalBytesRead, sourceOffset, bytesRead, buf.length));
      outStream.close();
      outStream = null;
    } finally {
  ...
  /**
   * let readBytes will try to modify the current pos of the stream, unified the read meaning.
   * however for PositionedReadable, this will not modify the postion.
   * @param inStream
   * @param buf
   * @param position
   * @return
