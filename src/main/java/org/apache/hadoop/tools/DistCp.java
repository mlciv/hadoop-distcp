...
import org.apache.hadoop.tools.mapred.filechunk.CopyChunkMapper;
...
public class DistCp extends Configured implements Tool {
  ...
  private Job createJob() throws IOException {
    ...
    //TODO: maxThreads<0, no parallel
    if(inputOptions.getMaxThreads()<0){
      Class<? extends Mapper> mapperClass;
      if (inputOptions.getByChunk()) {
        mapperClass = CopyChunkMapper.class;
        job.setNumReduceTasks(4);
        // for chunk add reduce task for stitch
        //TODO:
      } else {
        mapperClass = CopyMapper.class;
      ...
      if(inputOptions.getByChunk()){
        MultithreadedMapper.setMapperClass(job, CopyChunkMapper.class);
      }else{
        MultithreadedMapper.setMapperClass(job,CopyMapper.class);
      }
      MultithreadedMapper.setNumberOfThreads(job, inputOptions.getMaxThreads());
    }
