...
public class OptionsParser {
  ...
  public static DistCpOptions parse(String args[]) throws IllegalArgumentException {
    ...
    if (command.hasOption(DistCpOptionSwitch.BY_CHUNK.getSwitch())){
      option.setByChunk(true);
    }

    if (command.hasOption(DistCpOptionSwitch.MAX_THREADS.getSwitch())) {
      try {
        Integer maxThreads = Integer.parseInt(
                getVal(command, DistCpOptionSwitch.MAX_THREADS.getSwitch()).trim());
        option.setMaxThreads(maxThreads);
      } catch (NumberFormatException e) {
        throw new IllegalArgumentException("Number of threads is invalid: " +
                getVal(command, DistCpOptionSwitch.MAX_THREADS.getSwitch()), e);
      }
    }
