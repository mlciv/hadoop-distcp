...
import org.apache.hadoop.tools.mapred.filechunk.UniformChunkSizeInputFormat;
...
public class DistCpUtils {
  ...
  public static int getBlocksPerChunk(Configuration configuration){
    int blocksPerChunk = configuration.getInt(DistCpConstants.CONF_LABEL_BLOCKS_PER_CHUNK, DistCpConstants.DEFAULT_BLOCKS_PER_CHUNK);
    if (blocksPerChunk <= 0) {
      LOG.warn(DistCpConstants.CONF_LABEL_BLOCKS_PER_CHUNK +
              " should be positive. Fall back to default value: "
              + DistCpConstants.CONF_LABEL_BLOCKS_PER_CHUNK);
      blocksPerChunk = DistCpConstants.DEFAULT_BLOCKS_PER_CHUNK;
    }
   ...
   */
  //TODO: Test Options strategy and chunk
  public static Class<? extends InputFormat> getStrategy(Configuration conf,
    ...
    if(!options.getByChunk()){
      ...
      String confLabel = "distcp.bychunk." +
              options.getCopyStrategy().toLowerCase(Locale.getDefault()) + ".strategy.impl";
      return conf.getClass(confLabel, UniformChunkSizeInputFormat.class, InputFormat.class);
    }
  }

//  /**
//   * Returns the class that implements a Mapper. Looks up the implementation for
//   * a particular strategy from distcp-default.xml
//   *
//   * @param conf - Configuration object
//   * @param options - Handle to input options
...
//                                                         DistCpOptions options) {
//    if(!options.getByChunk()){
//      String confLabel = "distcp.mapper.impl";
//      return conf.getClass(confLabel, CopyMapper.class, Mapper.class);
//    }else {
//      String confLabel = "distcp.bychunk.mapper.impl";
//      return conf.getClass(confLabel, CopyChunkMapper.class, Mapper.class);
//    }
//  }
  ...
  /**
   * Gets relative chunk path of child path with respect to a root path
   * For ex. If childPath = /tmp/abc/xyz/file and
   *            sourceRootPath = /tmp/abc
   *            chunkIndex =0
   *            chunkNum = 5
   * Relative path would be /xyz/file_0000000005_chunkfiles/0000000000
   *         If childPath = /file and
   *            sourceRootPath = /
   * Relative path would be /file_0000000005_chunkfiles/0000000000
   * @param sourceRootPath - Source root path
   * @param childPath - Path for which relative path is required
   * @return - Relative portion of the child path (always prefixed with /
   *           unless it is empty
   */
  public static String getChunkRelativePath(Path sourceRootPath, Path childPath,int chunkIndex,int chunkNum){
    String originRelPath = getRelativePath(sourceRootPath,childPath);
    StringBuilder sb = new StringBuilder(originRelPath);
    //int.max 2147483648 10 chars
    sb.append(String.format("_%010d_chunkfiles/%010d",chunkNum,chunkIndex));
    return sb.toString();
  }
   ...
   *           unless it is empty
   */
  public static String changeOriginalPathToChunk(String originRelPath,int chunkIndex,int chunkNum){
    StringBuilder sb = new StringBuilder(originRelPath);
    //int.max 2147483648 10 chars
    sb.append(String.format("_%010d_chunkfiles/%010d",chunkNum,chunkIndex));
    return sb.toString();
  ...
  /**
   * filename_%010d_chunkfiles/%10d = adding 33 chars behind real filename
   * @param chunkRelPath
   * @return
   */
  public static String changeChunkToOriginalPath(String chunkRelPath) throws IOException {
    int len = chunkRelPath.length();
    if(len < 33){
      LOG.error("invalid chunkRelPath" + chunkRelPath);
      throw new IOException("invalid chunkRelPath"+ chunkRelPath);
    }
    int end = len - 33;
    return chunkRelPath.substring(0,end);
  }
  ...
  public static boolean compareFileLengths(FileStatus sourceFileStatus, Path target,
                                     Configuration configuration, long targetLen)
          throws IOException {
    final Path sourcePath = sourceFileStatus.getPath();
    FileSystem fs = sourcePath.getFileSystem(configuration);
    if (fs.getFileStatus(sourcePath).getLen() != targetLen){
      LOG.error("Mismatch in length of source:" + sourcePath
              + " and target:" + target);
      return false;
    }
    return true;
  ...
  public static boolean compareCheckSums(FileSystem sourceFS, Path source,
                                   FileChecksum sourceChecksum, FileSystem targetFS, Path target)
          throws IOException {
