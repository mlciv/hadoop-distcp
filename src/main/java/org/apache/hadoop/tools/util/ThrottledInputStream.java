...
public class ThrottledInputStream extends InputStream {
   ...
   * What's more, this read Postion will try to modified the Current Postion of the stream,
   * but not modified the stream for only positionedReadable
   */
  public int read(long position, byte[] buffer, int offset, int length)
      throws IOException {
    if (!(rawStream instanceof PositionedReadable)&&!(rawStream instanceof Seekable)) {
      throw new UnsupportedOperationException(
          "positioned or seek read is not supported by the internal stream");
    }
    throttle();
    int readLen = 0;
    if(rawStream instanceof Seekable){
      //seek read at first, seek will modify the current position.
      if(position!=((Seekable)rawStream).getPos()){
        // not equal to current position, then seek and read.
        ((Seekable)rawStream).seek(position);
      }
      readLen = rawStream.read(buffer,offset,length);
    }else{
      // read position will not modify the current position.
      readLen = ((PositionedReadable) rawStream).read(position, buffer,
              offset, length);
    }
    if (readLen != -1) {
      bytesRead += readLen;
