...
public class SimpleCopyListing extends CopyListing {
  ...
  @VisibleForTesting
  public SimpleCopyListing(Configuration configuration) {
  ...
  public void buildListing(Path pathToListFile,
    ...
    Path pathToCheckFile = new Path(pathToListFile.toString()+"_forchecking");
    validatePaths(options);
    doBuildListing(pathToCheckFile, options);
    validateFinalListing(pathToCheckFile, options);

    //TODO: for chunk and chunkParallel
    postBuildListing(pathToCheckFile,pathToListFile,options);
  }
